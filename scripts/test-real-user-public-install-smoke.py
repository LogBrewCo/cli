...
class PublicInstallVerifierTests(unittest.TestCase):
    ...
    def artifact_for(self, mode: str) -> tuple[str, pathlib.Path]:
        ...
        if mode == "crates":
            ...
            create_tar(
                artifact,
                {
                    f"logbrew-cli-{VERSION}/Cargo.toml": (
                        f'[package]\nname = "logbrew-cli"\nversion = "{VERSION}"\n'.encode(),
                        0o644,
                    ),
                    f"logbrew-cli-{VERSION}/Cargo.lock": (b"# fixture\n", 0o644),
                    f"logbrew-cli-{VERSION}/src/main.rs": (b"fn main() {}\n", 0o644),
                },
            )
        ...
        if mode == "native":
            ...
            create_tar(
                artifact,
                {f"logbrew-{VERSION}/logbrew": (cli_source(VERSION).encode(), 0o755)},
            )
            ...
            create_tar(
                artifact,
                {
                    "package/package.json": (
                        (
                            f'{{"name":"logbrew-cli","version":"{VERSION}",'
                            '"bin":{"logbrew":"run-logbrew.js"}}\n'
                        ).encode(),
                        0o644,
                    ),
    ...
    def test_homebrew_formula_rejects_mixed_derived_release_versions(self) -> None:
        ...
        artifact.write_text(
            source.replace(
                f"v{VERSION}/logbrew-cli-x86_64",
    ...
    def test_homebrew_formula_accepts_one_exact_explicit_version(self) -> None:
        matcher = load_verifier_module().homebrew_formula_matches_version

        exact = f'version "{VERSION}"\n'
        self.assertTrue(matcher(exact, VERSION))
        self.assertFalse(matcher('version "0.1.28"\n', VERSION))
        self.assertFalse(matcher(exact * 2, VERSION))
