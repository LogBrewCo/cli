...
VERSION = "0.1.34"
EXPECTED_RELEASE_VERSION = "0.1.34"
...
class PublicInstallVerifierTests(unittest.TestCase):
    ...
    def artifact_for(self, mode: str) -> tuple[str, pathlib.Path]:
        ...
        if mode == "crates":
            ...
            create_tar(
                artifact,
                {
                    "logbrew-cli-0.1.34/Cargo.toml": (
                        b'[package]\nname = "logbrew-cli"\nversion = "0.1.34"\n',
                        0o644,
                    ),
                    "logbrew-cli-0.1.34/Cargo.lock": (b"# fixture\n", 0o644),
                    "logbrew-cli-0.1.34/src/main.rs": (b"fn main() {}\n", 0o644),
                },
            )
        ...
        if mode == "native":
            ...
            create_tar(
                artifact,
                {"logbrew-0.1.34/logbrew": (cli_source(VERSION).encode(), 0o755)},
            )
            ...
            create_tar(
                artifact,
                {
                    "package/package.json": (
                        b'{"name":"logbrew-cli","version":"0.1.34",'
    ...
    def test_homebrew_formula_accepts_one_exact_explicit_version(self) -> None:
        ...
        self.assertTrue(matcher('version "0.1.34"\n', VERSION))
