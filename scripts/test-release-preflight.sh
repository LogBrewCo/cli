...
cat >"$tmp_dir/dist-homebrew-formula-smoke" <<'STUB'
#!/usr/bin/env bash
set -euo pipefail

if [[ "${1:-}" != "v0.1.0" ]]; then
  printf 'unexpected dist Homebrew formula tag: %s\n' "${1:-}" >&2
  exit 1
fi

if [[ "${LOGBREW_TEST_DIST_HOMEBREW_FORMULA:-pass}" == "fail" ]]; then
  printf 'test dist Homebrew formula failed\n' >&2
  exit 1
fi

printf 'Dist Homebrew formula smoke passed: test\n'
STUB
...
cat >"$tmp_dir/dist-checksums" <<'STUB'
#!/usr/bin/env bash
set -euo pipefail

if [[ "${1:-}" != "v0.1.0" ]]; then
  printf 'unexpected dist checksum tag: %s\n' "${1:-}" >&2
  exit 1
fi

if [[ "${LOGBREW_TEST_DIST_CHECKSUMS:-pass}" == "fail" ]]; then
  printf 'test dist checksums failed\n' >&2
  exit 1
fi

printf 'Dist checksum check passed: test\n'
STUB
...
cat >"$tmp_dir/dist-shell-installer-smoke" <<'STUB'
#!/usr/bin/env bash
set -euo pipefail

if [[ "${1:-}" != "v0.1.0" ]]; then
  printf 'unexpected dist shell installer tag: %s\n' "${1:-}" >&2
  exit 1
fi

if [[ "${LOGBREW_TEST_DIST_SHELL_INSTALLER:-pass}" == "fail" ]]; then
  printf 'test dist shell installer failed\n' >&2
  exit 1
fi

printf 'Dist shell installer smoke passed: test\n'
STUB
...
chmod +x "$tmp_dir/cargo" "$tmp_dir/cargo-audit" "$tmp_dir/curl" "$tmp_dir/gh" "$tmp_dir/git" "$tmp_dir/package-install-smoke" "$tmp_dir/dist-plan" "$tmp_dir/dist-global-artifacts" "$tmp_dir/dist-homebrew-formula-smoke" "$tmp_dir/dist-source-artifact-smoke" "$tmp_dir/dist-checksums" "$tmp_dir/dist-local-artifacts" "$tmp_dir/dist-npm-package-install-smoke" "$tmp_dir/dist-shell-installer-smoke"
...
export LOGBREW_RELEASE_DIST_HOMEBREW_FORMULA_SCRIPT="$tmp_dir/dist-homebrew-formula-smoke"
export LOGBREW_RELEASE_DIST_SOURCE_ARTIFACT_SCRIPT="$tmp_dir/dist-source-artifact-smoke"
export LOGBREW_RELEASE_DIST_CHECKSUMS_SCRIPT="$tmp_dir/dist-checksums"
...
: >"$output_file"
if LOGBREW_TEST_SECRETS=all LOGBREW_TEST_DIST_HOMEBREW_FORMULA=fail PATH="$tmp_dir:$PATH" bash scripts/release-preflight.sh v0.1.0 >"$output_file" 2>&1; then
  printf 'expected release preflight to fail when dist Homebrew formula smoke fails\n' >&2
  cat "$output_file" >&2
  exit 1
fi

expected_homebrew_formula_lines=(
  "test dist Homebrew formula failed"
  "Release preflight failed: cargo-dist Homebrew formula smoke failed"
  "Next: fix cargo-dist Homebrew formula installability, then rerun bash scripts/test-dist-homebrew-formula-smoke.sh and scripts/release-preflight.sh v0.1.0 before tagging."
)

for line in "${expected_homebrew_formula_lines[@]}"; do
  if ! grep -Fq "$line" "$output_file"; then
    printf 'expected dist Homebrew formula output to contain: %s\n' "$line" >&2
    printf 'actual output:\n' >&2
    cat "$output_file" >&2
    exit 1
  fi
...
: >"$output_file"
if LOGBREW_TEST_SECRETS=all LOGBREW_TEST_DIST_CHECKSUMS=fail PATH="$tmp_dir:$PATH" bash scripts/release-preflight.sh v0.1.0 >"$output_file" 2>&1; then
  printf 'expected release preflight to fail when dist checksum check fails\n' >&2
  cat "$output_file" >&2
  exit 1
fi

expected_checksum_lines=(
  "test dist checksums failed"
  "Release preflight failed: cargo-dist checksum verification failed"
  "Next: fix cargo-dist checksum artifacts, then rerun bash scripts/test-dist-checksums.sh and scripts/release-preflight.sh v0.1.0 before tagging."
)

for line in "${expected_checksum_lines[@]}"; do
  if ! grep -Fq "$line" "$output_file"; then
...
: >"$output_file"
if LOGBREW_TEST_SECRETS=all LOGBREW_TEST_DIST_SHELL_INSTALLER=fail PATH="$tmp_dir:$PATH" bash scripts/release-preflight.sh v0.1.0 >"$output_file" 2>&1; then
  printf 'expected release preflight to fail when dist shell installer smoke fails\n' >&2
  cat "$output_file" >&2
  exit 1
fi

expected_shell_installer_lines=(
  "test dist shell installer failed"
  "Release preflight failed: cargo-dist shell installer smoke failed"
  "Next: fix cargo-dist shell installer installability, then rerun bash scripts/test-dist-shell-installer-smoke.sh and scripts/release-preflight.sh v0.1.0 before tagging."
)

for line in "${expected_shell_installer_lines[@]}"; do
  if ! grep -Fq "$line" "$output_file"; then
