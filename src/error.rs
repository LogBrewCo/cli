...
pub enum RuntimeError {
    ...
    /// A successful product-analytics property catalog violated its versioned public contract.
    #[error("product analytics property response is invalid")]
    AnalyticsPropertiesResponseInvalid,
...
const fn cli_error_next_step(error: &CliError) -> &'static str {
    match error {
        ...
        CliError::InvalidInvestigationCommand => {
            "use logbrew investigate issue <issue_id> or logbrew explain issue <issue_id> with optional --occurrence recommended|first|latest|<occurrence_id>, or append verify --baseline-occurrence <occurrence_id> --candidate-deployment <deployment_id>; both forms accept --json"
        }
...
const fn runtime_error_code(error: &RuntimeError) -> &'static str {
    match error {
        ...
        RuntimeError::AnalyticsPropertiesResponseInvalid => "analytics_properties_response_invalid",
...
const fn fallback_runtime_error_next_step(error: &RuntimeError) -> &'static str {
    match error {
        ...
        RuntimeError::AnalyticsPropertiesResponseInvalid => {
            "retry the same analytics property query; if it repeats, report the public response contract"
        }
        RuntimeError::AnalyticsResponseInvalid => {
