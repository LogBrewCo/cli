...
Issue explanations use the same schema-version-10 request, grouping, occurrence, lifecycle, exception, source-locator, impact, \
...
Reads one schema-version-12 bounded issue investigation with deterministic grouping, exact preceding-deployment and customer-source locator evidence, and privacy-bounded request evidence, plus explicit selected, first, latest, and \
...
coverage and limitations, trace, sibling issues, related logs, actions, metric exemplars, release scope, evidence \
...
validated schema-version-12 response for AI agents.
