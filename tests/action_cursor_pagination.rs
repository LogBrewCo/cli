...
fn action_cursor_flags_fail_closed_with_concrete_recovery() {
    ...
    let unsupported = parse_command(["logbrew", "logs", "--pagination", "cursor", "--json"])
        .expect_err("action pagination stays action-only");
    let mut output = Vec::new();
    ...
    assert_eq!(
        body["message"],
        "unsupported flag for read logs: --pagination"
    );
    assert_eq!(body["next"], "run logbrew read logs --help");
}
